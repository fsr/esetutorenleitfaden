...
\title{\huge{\textbf{Vor dem Tutorium lesen und wichtige Punkte markieren!}}\\\ \\\ \\{Leitfaden für ESE-Tutoren 2015}}
\date{}
...
Die Wanderung findet ein Wochenende später statt!\\ Bei Interesse bitte an Julius <julius@ifsr.de> wenden.
...
\begin{itemize*}
	...
	\begin{center}
	Am besten konkret an AuD und Mathe 1 zeigen und TGI erklären.\\
	Die Zeichnung an die Tafel malen oder was eigenes ausdenken.
	\end{center}
	\begin{tikzpicture}
		\draw (0,0) rectangle (4,-5);
		\node at (2,0) [below] {AuD};
		\draw (0,-.5) -- (4,-.5);
		\node (v1) [ellipse,draw] at (1.6,-1.5) {Vorlesung};
		\node (u1) [ellipse,draw] at (2.8,-2.7) {\"Ubung};
		\draw (0,-3.5) -- (4,-3.5);
		\node (p1) [ellipse,draw] at (2,-4.3) {Pr\"ufung};
		
		\draw (5,0) rectangle (12 ,-5);
		\node at (8.5,0) [below] {Mathe};
		\draw (5,-.5) -- (12,-.5);
		\node (pvl) [rectangle,draw,font=\scriptsize] at (8.5,-3.3) [above]{Pr\"ufungsvorleistung};
		\draw (8.5,-.5) -- (pvl);
		\node at (6.75,-.5) [below] {LAG};
		\node at (10.25,-.5) [below] {DIS};
		\draw (5,-1) -- (12, -1);
		\node (v2) [ellipse,draw] at (6.6,-1.6) {Vorlesung};
		\node (u2) [ellipse,draw] at (6.1,-2.7) {\"Ubung};
		\node (v3) [ellipse,draw] at (10.1,-1.6) {Vorlesung};
		\node (u3) [ellipse,draw] at (10.9,-2.7) {\"Ubung};
		\draw (5,-3.3) -- (12,-3.3);
		\node (p2) [ellipse,draw] at (7.35,-4) {Nikolauspr\"ufung};
		\node (p3) [ellipse,draw] at (10.7,-4.4) {Pr\"ufung};
		\draw[->,thick] (pvl) -- (p3);
	\end{tikzpicture}
...
\subsection{Vorlesung, Übung, Praktikas}
\begin{itemize*}
...
\begin{itemize*}
\item \textbf{Aktuelles Semester:} wegen Flüchtlingssituation nicht komplettes Sportangebot
...
\item Preise für Studenten recht günstig (zwischen ca. 15-40 Euro pro Semester)
